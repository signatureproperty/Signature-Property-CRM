...
import { format, parseISO, isAfter, subMonths, subDays } from 'date-fns';
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <Card className="border-none shadow-xl bg-card/60 backdrop-blur-sm">
                    ...
                    <CardContent>
                        ...
                        <p className="text-[10px] text-muted-foreground mt-1 font-bold flex items-center gap-1">Added in range</p>
                    </CardContent>
                </Card>
                <Card className="border-none shadow-xl bg-card/60 backdrop-blur-sm">
                    <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
            ...
            <Tabs defaultValue="overview" className="w-full">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        <Card className="lg:col-span-2 border-none shadow-2xl rounded-2xl overflow-hidden bg-card/60 backdrop-blur-xl">
                            ...
                            <CardContent className="h-[400px]">
                                <ResponsiveContainer width="100%" height="100%">
                                    <AreaChart data={[
                                        { name: 'Jan', inv: 400, leads: 240 },
                                        { name: 'Feb', inv: 300, leads: 139 },
                                        { name: 'Mar', inv: 200, leads: 980 },
                                        { name: 'Apr', inv: 278, leads: 390 },
                                        { name: 'May', inv: 189, leads: 480 },
                                    ]}>
                                        <CartesianGrid strokeDasharray="3 3" vertical={false} />
                                        <XAxis dataKey="name" axisLine={false} tickLine={false} />
                                        <YAxis axisLine={false} tickLine={false} />
                                        <Tooltip />
                                        <Area type="monotone" dataKey="inv" stackId="1" stroke="#2563eb" fill="#2563eb" fillOpacity={0.1} />
                                        <Area type="monotone" dataKey="leads" stackId="1" stroke="#10b981" fill="#10b981" fillOpacity={0.1} />
                                    </AreaChart>
                                </ResponsiveContainer>
                            </CardContent>
                        </Card>
                        ...
                        <Card className="border-none shadow-2xl rounded-2xl bg-card/60 backdrop-blur-xl">
                            ...
                            <CardContent className="h-[400px]">
                                <ResponsiveContainer width="100%" height="100%">
                                    <PieChart>
                                        <Pie data={propertyChartData} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    {agentStats.map((agent) => (
                                        <TableRow key={agent.id} className="group hover:bg-primary/5">
                                            ...
                                            <TableCell className="text-right">
                                                <Button size="sm" variant="ghost" className="rounded-full h-8 text-[10px] font-black uppercase tracking-widest gap-2" onClick={() => setSelectedAgentForReport(agent)}>
                ...
                <TabsContent value="appointments" className="mt-6">
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        <Card className="lg:col-span-2 border-none shadow-2xl rounded-2xl overflow-hidden bg-card/60 backdrop-blur-xl">
                            <CardHeader>
                                <CardTitle className="text-sm font-black uppercase tracking-wider">Appointment Status Breakdown</CardTitle>
                            </CardHeader>
                            <CardContent className="h-[400px]">
                                <ResponsiveContainer width="100%" height="100%">
                        ...
                        <Card className="border-none shadow-2xl rounded-2xl bg-card/60 backdrop-blur-xl">
                             <CardHeader>
                                <CardTitle className="text-sm font-black uppercase tracking-wider">Upcoming Schedule</CardTitle>
                            </CardHeader>
                            <CardContent>
                                <div className="space-y-4">
                                    {appointments?.filter(a => a.status === 'Scheduled').slice(0, 5).map(a => (
                                        <div key={a.id} className="flex items-center gap-3 p-3 rounded-xl bg-muted/20 border border-border/40">
                                            <div className="h-8 w-8 rounded-full bg-primary/10 flex items-center justify-center text-primary"><Calendar className="h-4 w-4" /></div>
                                            <div className="flex-1 min-w-0">
                                                <p className="text-sm font-bold truncate">{a.contactName}</p>
                                                <p className="text-[10px] text-muted-foreground">{a.date} at {a.time}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                ...
                <TabsContent value="properties" className="mt-6">
                    <Card className="border-none shadow-2xl rounded-2xl overflow-hidden bg-card/60 backdrop-blur-xl">
                        <CardHeader>
                            <CardTitle className="text-sm font-black uppercase tracking-wider">Inventory Analysis</CardTitle>
                        </CardHeader>
                        <CardContent className="h-[400px]">
                            <ResponsiveContainer width="100%" height="100%">
                ...
                <TabsContent value="buyers" className="mt-6">
                    <Card className="border-none shadow-2xl rounded-2xl overflow-hidden bg-card/60 backdrop-blur-xl">
                        <CardHeader>
                            <CardTitle className="text-sm font-black uppercase tracking-wider">Buyer Conversion Metrics</CardTitle>
                        </CardHeader>
                        <CardContent className="h-[400px] flex items-center justify-center text-muted-foreground">
                            <Users className="h-12 w-12 opacity-20" />
                            <span className="ml-4 font-bold">Analysis details coming soon for high-volume accounts.</span>
                        </CardContent>
                    </Card>
                </TabsContent>
            </Tabs>
            ...
            <Dialog open={!!selectedAgentForReport} onOpenChange={() => setSelectedAgentForReport(null)}>
                <DialogContent className="sm:max-w-3xl max-h-[90vh] flex flex-col p-0 overflow-hidden">
                    <DialogHeader className="hidden">
                        <DialogTitle>Agent Report</DialogTitle>
                    </DialogHeader>
                                                ...
                                                <div key={act.id} className="relative pl-10">
                                                    ...
                                                    <div className="bg-card border border-border/60 p-4 rounded-2xl shadow-sm">
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <Badge className="text-[10px] bg-emerald-500 text-white">{act.details?.to}</Badge>
                                                        </div>
                                                    </div>
                                                </div>
                                            )) : (
