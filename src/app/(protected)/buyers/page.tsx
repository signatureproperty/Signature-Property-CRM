...
function BuyersPageContent() {
    ...
    const allAreas = useMemo(() => {
        ...
        allBuyers.forEach((b) => {
            if (b.area_preference) {
                b.area_preference.split(',').forEach(a => areas.add(a.trim()));
            }
        });
    ...
    const filteredBuyers = useMemo(() => {
        ...
        if (filters.area.length > 0) buyers = buyers.filter(b => filters.area.some(a => (b.area_preference || '').split(',').map(x => x.trim()).includes(a)));
                                                    ...
                                                    <PopoverTrigger asChild>
                                                        <Button variant="outline" className="w-full justify-between text-sm font-normal h-8">
                                                            {filters.area.length > 0 ? (
                                                                <span className="text-primary">{filters.area.length} selected</span>
                                                            ) : (
