...
export function PropertyNotesDialog({
  ...
  useEffect(() => {
    if (isOpen) {
      setLocalNotes(property.timeline_notes || []);
    }
  }, [isOpen, property.timeline_notes]);
      ...
      <DialogContent className="sm:max-w-2xl max-h-[70vh] sm:max-h-[90vh] flex flex-col p-0 overflow-hidden">
        ...
        <DialogFooter className="p-4 bg-muted/30 border-t flex items-center justify-between sm:justify-between">
          <div className="flex items-center gap-2">
            {profile.role === 'Admin' && sortedNotes.length > 0 && (
              <AlertDialog>
                <AlertDialogTrigger asChild>
                  <Button variant="ghost" size="sm" className="h-9 rounded-full gap-2 text-destructive hover:bg-destructive/10 font-bold px-3">
            ...
            <Button variant="outline" size="sm" className="rounded-full h-9 gap-2 font-bold px-3" onClick={() => setIsDetailsOpen(true)}>
